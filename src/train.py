from utils import update_model, save_simle_metrics_report, get_model_performance_test_set
...
logger.info('Settin Hyperparameter to tune')
param_tuning = {'core_model__n_estimators': range(20, 401, 20)}
...
logger.info('Cross validatin with the best model...')
final_result = cross_validate(grid_search.best_estimator_, X_train, y_train, return_train_score=True, cv=5)

train_score = np.mean(final_result['train_score']) 
test_score = np.mean(final_result['test_score']) 
# creamos estos assertion, basados en los criterios con los que se cree el modelo
...
save_simle_metrics_report(train_score, test_score, validation_score, grid_search.best_estimator_)
